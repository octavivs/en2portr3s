<div id="services">
    <div class="row">
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Playeras"/>
            <img src="view/img/playeras.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Mouse pack"/>
            <img src="view/img/mouse_pack.png" class="hidden-for-small"/>
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Platos"/>
            <img src="view/img/platos.png" class="hidden-for-small" />
        </div>
    </div>

    <div class="row">
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Rompecabezas"/>
            <img src="view/img/rompecabezas.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Almohadas" />
            <img src="view/img/almohadas.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Tazas"/>
            <img src="view/img/tazas.png" class="hidden-for-small" />
        </div>
    </div>

    <div class="row">
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Fotobotones"/>
            <img src="view/img/fotobotones.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Flyers digital"/>
            <img src="view/img/flyers_digital.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Flyers 1 tinta"/>
            <img src="view/img/flyers_1_tinta.png" class="hidden-for-small" />
        </div>
    </div>

    <div class="row">
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Tarjetas"/>
            <img src="view/img/tarjetas.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Credenciales 4x1"/>
            <img src="view/img/credenciales_4x1.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Credenciales 4x4"/>
            <img src="view/img/credenciales_4x4.png" class="hidden-for-small" />
        </div>
    </div>

    <div class="row">
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Serigrafía"/>
            <img src="view/img/serigrafia.png" class="hidden-for-small" />
        </div>
        <div class="large-4 columns">
            <input type="button" class="button expand" value="Lonas"/>
            <img src="view/img/lonas.jpg" class="hidden-for-small" />
        </div>
    </div>
</div>
